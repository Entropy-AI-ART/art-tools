...
content_v = torch.Tensor(1, 3, 512, 512)
style_v = torch.Tensor(1, 3, 512, 512)
vgg = Encoder4()
dec = Decoder4()
matrix = MulLayer('r41')
...
def fusion():
    ...
    content_dataset = Dataset(upload, 512, 512, test=True)
    content_loader = torch.utils.data.DataLoader(dataset=content_dataset,
                                                 batch_size=1,
                                                 shuffle=False,
                                                 num_workers=1)
    style_dataset = Dataset(gen_image, 512, 512, test=True)
